...
class Salesperson extends Staff {
    ...
    Vehicle sellVehicle(Buyer b, ArrayList<Vehicle> vList) {
        ...
        /*Tyler Learned
            addons
                extendedWarranty    chance = .25, price + (price*.2)
                undercoating        chance = .10, price + (price*.05)
                roadRescueCoverage  chance = .05, price + (price*.02)
                satteliteRadio      chance = .40, price + (price*.05)
        End Tyler Learned*/
    }
...
class Driver extends Staff{
    static List<String> names = Arrays.asList("Erik","Matt","Anna");
    static Namer namer = new Namer(names);
    Driver() {
        super();
    ...
    void race() {
        //if win get bonus
        //if palce 15-20 driverStatus injured .3
    }  
}
